...
libraries_raw <- read.csv(library_source_url, sep = ";", check.names = FALSE, fileEncoding = "UTF-8-BOM")
library_columns <- c(
  "Bibliothèque ou Centre régional", "Région administrative", "Population desservie",
  "Prêts / Tous les doc. (Total)", "Progr. / Toutes les activités (Total)", "Visites (Total)"
)
...
names(libraries) <- c("bibliotheque", "region", "population_desservie", "prets", "activites", "visites")

portal_raw <- readr::read_csv(portal_source_url, show_col_types = FALSE)
portal <- portal_raw[
...
for (output_directory in output_directories) {
  readr::write_csv(libraries, file.path(output_directory, "bibliotheques_publiques_quebec_2024.csv"), na = "")
  readr::write_csv(portal, file.path(output_directory, "frequentation_portail_montreal_2023.csv"), na = "")
}
