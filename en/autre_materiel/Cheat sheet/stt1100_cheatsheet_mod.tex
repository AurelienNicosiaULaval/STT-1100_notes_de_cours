...
\urlstyle{same}
\hypersetup{
  pdftitle={STT-1100 --- Checklist sheet},
  pdfauthor={Laval University},
  pdflang={en},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
...
\section{ WORK ENVIRONMENT}\label{work-environment}
...
\item
  Render Quarto: \texttt{Ctrl\ +\ Shift\ +\ K}
\end{itemize}
...
\begin{Highlighting}[]
\CommentTok{{-}{-}{-}}
\AnnotationTok{title:}\CommentTok{ "Title"}
...
\subsection{Basic packages}\label{basic-packages}
...
\section{\texorpdfstring{ MANIPULATE DATA ---
\texttt{dplyr}}{ MANIPULATE DATA --- dplyr}}\label{manipulate-data-dplyr}
...
\subsection{Group and summarize}\label{group-and-summarize}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{group\_by}\NormalTok{(df, cat)}
\FunctionTok{summarise}\NormalTok{(df, }\AttributeTok{avg =} \FunctionTok{mean}\NormalTok{(x, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{\texorpdfstring{ VISUALIZATION ---
\texttt{ggplot2}}{ VISUALIZATION --- ggplot2}}\label{visualization-ggplot2}
...
\end{Shaded}
 ...
 Use \texttt{facet\_wrap()} to compare multiple groups

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{ DESCRIPTIVE ANALYSIS}\label{descriptive-analysis}

\subsection{Digital statistics}\label{digital-statistics}
...
\section{ GOOD PRACTICE TIPS}\label{good-practice-tips}

\begin{itemize}
\tightlist
\item
  \texttt{glimpse(df)}: structure preview
\item
  \texttt{head(df)} / \texttt{tail(df)}
\item
  Comments \texttt{\#} to clarify
\item
