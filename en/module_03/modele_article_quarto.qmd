---
title: "Quarto model: make an article readable"
subtitle: "Challenge 3 • hide code, choose outputs and tell the story"
author:
...
Inline code written as <code>r expression</code> inside Quarto delimiters keeps the number synchronized with the analysis. Do not type a calculated result manually into the text: if the data or filter changes, the manual text can become wrong.

An example actually calculated on this page: the dataset contains **`r nrow(condamnations)` findings** and **`r n_distinct(na.omit(condamnations$SOC_NOM_ARTCL_INFRC))` non-missing categories**. The reader sees the sentence and numbers, not the code that produces them.
...
```{r}
#| label: fig-article
#| fig-cap: "A chart should be followed by a sentence stating its main message."
#| fig-alt: "Horizontal bar chart showing the six most frequent offence categories."
#| fig-width: 8
#| fig-height: 5
results |>
  slice_head(n = 6) |>
  ...
  geom_col(fill = "#176b87") +
  coord_flip() +
  labs(x = NULL, y = "Number of findings", title = NULL) +
...
::: {.callout-tip title="A useful test"}
