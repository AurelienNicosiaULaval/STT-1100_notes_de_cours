...
Exécutez ce bloc d’installation une fois dans la Console de RStudio. L’archive publique GitHub ne nécessite aucun jeton d’authentification GitHub. Le package n’est pas encore sur le CRAN.
...
Si une ancienne commande `install_github()` échoue avec `HTTP error 401. Bad credentials`, reprenez le bloc d’installation ci-dessus. Il conserve vos identifiants GitHub enregistrés.
