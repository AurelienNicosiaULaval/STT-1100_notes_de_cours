...
- charger les données dans R avec la fonction [`read_csv()`](https://readr.tidyverse.org/reference/read_delim.html) du package `readr`,
...
Pensez à toujours vérifier le résultat de la transformation avec `glimpse()` ou [`summary()`](https://rdrr.io/r/base/summary.html).
...
Bien sur, il faut que ta variable soit de type `factor` pour que cette fonction fonctionne. Si ce n’est pas le cas, vous pouvez la convertir avec [`as.factor()`](https://rdrr.io/r/base/factor.html) (voir section précédente).
