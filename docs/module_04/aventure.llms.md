...
- charger les données dans R avec la fonction [`read_delim()`](https://readr.tidyverse.org/reference/read_delim.html) du package `readr`,
- examiner les premières lignes pour repérer les incohérences évidentes,
- et convertir immédiatement les noms de colonnes en minuscules avec [`janitor::clean_names()`](https://sfirke.github.io/janitor/reference/clean_names.html) pour faciliter les manipulations à venir.
...
L’une des fonctions les plus utiles ici est [`glimpse()`](https://pillar.r-lib.org/reference/glimpse.html) (du package `dplyr`). Elle affiche un aperçu des premières valeurs de chaque variable **ainsi que leur type** : par exemple `chr` pour une chaîne de caractères (character), `dbl` pour un nombre décimal (double), `int` pour un entier, ou encore `lgl` pour un booléen.
...
Pensez à toujours vérifier le résultat de la transformation avec [`glimpse()`](https://pillar.r-lib.org/reference/glimpse.html) ou [`summary()`](https://rdrr.io/r/base/summary.html).
...
Pour voir l’ensemble des niveaux d’un facteur, vous pouvez utiliser la fonction [`levels()`](https://rdrr.io/r/base/levels.html) :
...
Bien sûr, il faut que votre variable soit de type `factor` pour que cette fonction fonctionne. Si ce n’est pas le cas, vous pouvez la convertir avec [`as.factor()`](https://rdrr.io/r/base/factor.html) (voir section précédente).
...
- [`fct_count()`](https://forcats.tidyverse.org/reference/fct_count.html) : pour compter les niveaux et repérer les anomalies
- [`fct_recode()`](https://forcats.tidyverse.org/reference/fct_recode.html) : pour renommer des niveaux (par exemple fusionner `"CAR"` et `"car"`)
- [`fct_collapse()`](https://forcats.tidyverse.org/reference/fct_collapse.html) : pour fusionner plusieurs niveaux en un seul
...
Prenez chaque variable numérique et résumez-la avec [`summary()`](https://rdrr.io/r/base/summary.html) ou un boxplot pour identifier les extrêmes.
