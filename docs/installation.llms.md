...
if (length(packages_manquants) > 0) {
  install.packages(packages_manquants, repos = "https://cloud.r-project.org")
}
...
`UlavalSSD` n’est pas encore sur le CRAN. L’archive publique évite les erreurs d’authentification de l’API GitHub, dont `HTTP error 401. Bad credentials`, sans modifier vos identifiants enregistrés. Pour mettre à jour une installation existante, réexécutez seulement l’appel `remotes::install_url(...)` ci-dessus.
...
Le résultat attendu est `20111 11` : 20 111 lignes et 11 colonnes.
