...
> **NOTE:**
>
> Vous devez avoir reçu et accepté l’invitation à votre dépôt privé `aventure-2-VOTRE-IDENTIFIANT`. Si GitHub ou RStudio ne sont pas encore connectés, suivez la page [Connexion SSH entre GitHub et RStudio](../autre_materiel/connexion-ssh-github-rstudio.llms.md). Elle utilise principalement les menus et les boutons.
...
> **IMPORTANT:**
>
> Arrêtez-vous ici. Avant de fermer RStudio :
>
> 1.  vérifiez que votre nom et vos réponses sont dans `journal de bord.qmd`;
> 2.  rendez le journal afin de produire `journal de bord.html`;
> 3.  ouvrez le fichier HTML pour vérifier le résultat;
> 4.  faites un commit avec un message clair, puis un push vers GitHub.
>
> Vous n’avez pas à créer `rapport_analyse.qmd` maintenant. La suite commence à la section « Le retour de la Dre Adélie Fortier ».
