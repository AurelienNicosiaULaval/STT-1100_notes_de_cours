...
Proposition : 15 novembre 2026 à 23 h 55. Projet final : 14 décembre à 23 h 55. Brio demeure la référence officielle.
...
Le projet commence dans le dépôt privé remis à votre équipe. Ce dépôt est créé à partir du [template A26](https://github.com/STT-1100-A26/projet) et doit rester propre pendant toute la session.

Structure attendue :

Ouvrez `projet-stt1100.Rproj`, exécutez une fois `scripts/00_preparer.R` avec Source, puis cliquez sur Build \> Render Website. Chaque rendu reconstruit les données préparées avec `scripts/01_importer_preparer.R`. Adaptez ce script ainsi que le code de la proposition, du rapport et des diapositives à vos variables. Les données fictives du modèle ne peuvent pas servir au projet remis.
...
La présentation est un exposé de 7 minutes maximum par équipe, suivi d’une minute de transition vers l’équipe suivante. Chaque membre doit parler. Répétez en vous chronométrant et concentrez-vous sur les résultats essentiels; les détails restent dans le mini-site et le rapport.
