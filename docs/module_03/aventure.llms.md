# Aventure 3 - Infractions alimentaires dans le Grand Montréal

STT-1100 Introduction à la science des données

## Mise en situation : devenez journaliste de données

Vous êtes engagés comme **journaliste de données** par *Le Courrier Gourmand*, un média local fictif qui publie des enquêtes interactives sur l’alimentation dans le Grand Montréal. Votre rédactrice en chef souhaite un article éclairant le public sur **les infractions alimentaires commises dans les établissements du Grand Montréal**. Elle vous donne quelques pistes de réflexion :

- Quelle est la proportion des constats qui concernent des établissements situés dans le Grand Montréal ?
...
Organisation et contexte Média local fictif couvrant le Grand Montréal

Mission Produire un article clair sur les infractions alimentaires du Grand Montréal
...
Dans ce module, vous explorerez un **jeu de données** qui décrit diverses infractions, leurs amendes et le type d’établissements concernés. Votre mission : **analyser les constats d’infraction afin de produire un article clair sur les établissements alimentaires du Grand Montréal**.
...
# Cette nouvelle colonne ne modifie pas les valeurs brutes de Amende.
...
> **IMPORTANT:**
>
> *« Nous avons souvent parlé des constats associés au secteur `H2X`. Est-ce qu’ils représentent une grande part des constats du Grand Montréal ? »*
>
...
Dans cette aventure, le premier caractère H du code postal sert à définir le Grand Montréal. Postes Canada classe H dans la région du Grand Montréal, qui comprend notamment Laval et d’autres municipalités en plus de la Ville de Montréal. Les comparaisons ci-dessous portent donc sur le Grand Montréal postal, pas sur les seules limites de la Ville de Montréal. Consultez les [directives d’adressage de Postes Canada](https://www.canadapost-postescanada.ca/scp/fr/soutien/sujet/directives-adressage/codes-postaux.page) pour la définition des régions de tri.
...
condamnations <- listecondamnation %>%
  mutate(
    ...
    code_postal = str_to_upper(code_postal),
    code_postal = str_replace(code_postal, "^([A-Z][0-9][A-Z])\\s*([0-9][A-Z][0-9])$", "\\1 \\2"),
    est_grand_montreal = str_sub(code_postal, 1, 1) == "H",
    est_temp = coalesce(str_detect(SOC_NOM_ARTCL_INFRC, "TEMPERATURE"), FALSE),
    # Colonne numérique dérivée; la variable Amende reste inchangée.
    amende_num = parse_number(
      Amende,
      locale = locale(grouping_mark = " ", decimal_mark = ",")
    ),
...
condamnations_grand_montreal <- condamnations %>%
  filter(est_grand_montreal)
```

> **TIP:**
>
> 1.  **Filtrer le Grand Montréal** : Vérifiez que `condamnations_grand_montreal` ne contient que des constats dont le code postal commence par `H`.
>
...
>
> - Quelle est la proportion d’infractions reliées à la température dans le sous-ensemble du Grand Montréal ?
>
...
>
> - Quel est le montant moyen d’amende pour les infractions reliées à la température dans le sous-ensemble du Grand Montréal ?

> **IMPORTANT:**
>
> *« La colonne `SOC_NOM_ARTCL_INFRC` donne la classification de l’infraction. Combien de types d’infractions différentes avons-nous en lien avec la température dans le sous-ensemble du Grand Montréal ? »*
>
> - 1
>
> - 2
...
>
> Dans le sous-ensemble du Grand Montréal, on en compte 4 :
>
>     # A tibble: 4 × 1
>       SOC_NOM_ARTCL_INFRC
>       <chr>
...
>
> 3.  **Grand Montréal et reste du Québec** : Comparez la distribution entre les constats du Grand Montréal (`est_grand_montreal == TRUE`) et les autres constats. Quelle catégorie d’établissements semble la plus concernée ?

> **IMPORTANT:**
>
> On a toujours pensé que le Grand Montréal n’était pas le pire endroit pour les infractions alimentaires, peu importe le type de restaurant. Est-ce que vos résultats appuient cette idée ?
...
> **TIP:**
>
> 1.  **Tableau de contingence** : Construisez un tableau croisé entre `type_etablissement` et la nature de l’infraction (`SOC_NOM_ARTCL_INFRC`), dans votre sous-ensemble du Grand Montréal.
>
...
# Exemple : nombre d’infractions dans le Grand Montréal par type d’établissement
condamnations_grand_montreal %>%
  ...
  labs(
    title = "Infractions par type d’établissement dans le Grand Montréal",
    x = "Type d’établissement",
    y = "Nombre d'infractions"
  )
```

![Diagramme à barres montrant le nombre d'infractions dans le Grand Montréal par type d'établissement.](aventure_files/figure-html/unnamed-chunk-12-1.png)
...
Le **montant des amendes** est un indicateur concret qui attire souvent l’attention du public et des médias. Comparer ces montants entre le Grand Montréal et le reste du Québec, et entre types d’établissements, permettra d’appuyer votre conclusion.

> **TIP:**
>
> 1.  **Boîtes à moustaches** : Représentez la répartition du montant de l’amende au sein de chaque `type_etablissement`.
> 2.  **Comparaison** : Comparez le Grand Montréal au reste du Québec avec des boîtes à moustaches côte à côte, par type d’établissement. Que remarquez-vous ?
...
> **IMPORTANT:**
>
> *« Pense à citer un ou deux chiffres marquants dans ton texte : par exemple, la médiane des amendes dans le Grand Montréal par rapport au reste du Québec. Ça rendra tes conclusions plus percutantes. »*
...
- d’un **ensemble de résultats chiffrés** (top 5 infractions, montants moyens, proportion Grand Montréal vs reste du Québec, etc.) ;
