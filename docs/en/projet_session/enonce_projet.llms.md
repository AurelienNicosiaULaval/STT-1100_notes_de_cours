...
Proposal: November 15, 2026 at 11:55 p.m. Final project: December 14 at 11:55 p.m. Brio is the official reference.
...
The project starts in the private repository assigned to your team. This repository is created from the [A26 template](https://github.com/STT-1100-A26/projet) and should remain clean throughout the session.

Expected structure:

Open `projet-stt1100.Rproj`, run `scripts/00_preparer.R` once with Source, then click Build \> Render Website. Each render rebuilds the prepared data through `scripts/01_importer_preparer.R`. Adapt that script and the code in the proposal, report and slides to your own variables. The example data are fictitious and cannot be used for the submitted project.
...
Each team has a maximum of 7 minutes to present, followed by one minute to transition to the next team. Every member must speak. Rehearse with a timer and focus on the main findings; details belong in the website and report.
