...
- `challenge_04.qmd`: your reproducible working document;
- `clean_data.csv`: the cleaned dataset;
- `cleaning_log.Rdata`: an R object containing the `cleaning_log` list.

Your `.qmd` file must render to HTML without error.

# Minimal Log Structure

Your `cleaning_log` object must be a named list. Each entry must contain at least the fields `id`, `variables`, `problem`, `action` and `justification`.
...
5.  at least five documented entries in `cleaning_log`;
6.  a saved clean dataset;
7.  a saved `cleaning_log`.
...
- `clean_data.csv` is created by the code.
- `cleaning_log.Rdata` is created by the code.
