...
<html>
  <body>
    <div class="dataset-content">
      <h3 class="dataset-heading"><a>Exemple de jeu de données public</a></h3>
      <div class="dqc-org-cat">Organisation : Données Québec</div>
      <div class="dqc-org-cat">Catégorie : Santé</div>
    </div>
  </body>
</html>'
...
safe_read_lines <- function(url, fallback = character()) {
  tryCatch(
    ...
    error = function(error) {
      message("Impossible de télécharger ", url, "; utilisation d'un exemple local.")
      fallback
    }
  )
}
...
safe_read_html <- function(url) {
  tryCatch(
    read_html(url),
    error = function(error) {
      message("Impossible de télécharger ", url, "; utilisation d'un exemple HTML local.")
...
Aujourd'hui, vous êtes engagé·e par **Marie-Pier**, directrice de recherche à l'Institut québécois pour les données durables. Elle souhaite explorer le portail [donneesquebec.ca](https://www.donneesquebec.ca) pour identifier les jeux de données les plus récents et pertinents pour ses projets. Votre mission : **concevoir un outil de scraping fiable** pour extraire les titres, producteurs et catégories de jeux de données, tout en respectant les bonnes pratiques éthiques.
