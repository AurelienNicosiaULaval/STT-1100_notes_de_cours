...
- [R for Data Science - Strings](https://r4ds.hadley.nz/strings.html){target="_blank"}
  Pour manipuler des chaînes de caractères avec `stringr`: détecter un mot, extraire un motif, uniformiser la casse et nettoyer des espaces.

- [R for Data Science - Factors](https://r4ds.hadley.nz/factors.html){target="_blank"}
  Pour comprendre pourquoi l’ordre des modalités compte dans une analyse et dans un graphique.
...
- [Introduction to Modern Statistics - Explore categorical data](https://openintrostat.github.io/ims/explore-categorical){target="_blank"}
  Pour revoir les fréquences, les proportions, les tableaux croisés et les graphiques adaptés aux variables catégoriques.

- [Documentation de `forcats`](https://forcats.tidyverse.org/){target="_blank"}
  Pour regrouper des catégories rares, réordonner des modalités et rendre les graphiques plus lisibles.

### Aide-mémoires Posit

- [String manipulation with stringr :: Cheatsheet](https://rstudio.github.io/cheatsheets/strings.pdf){target="_blank"}
  Fonctions `stringr` pour détecter, extraire, remplacer et nettoyer du texte.

- [Factors with forcats :: Cheatsheet](https://rstudio.github.io/cheatsheets/factors.pdf){target="_blank"}
  Recodage, regroupement et réordonnancement de catégories.

- [Data visualization with ggplot2 :: Cheat Sheet](https://rstudio.github.io/cheatsheets/data-visualization.pdf){target="_blank"}
  Graphiques de catégories et réglages visuels fréquents.

Après les lectures, faites le [mini-test formatif](mini_test.qmd). Il n’est pas évalué, mais il vérifie les idées nécessaires pour l’aventure et le défi.
...
[Exercices du module 3](exercices.qmd)
